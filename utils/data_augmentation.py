...
def defor_3D_bc_in_batch(pc, R, t, s, model_point, nocs_scale):
    ...
    return pc_new, s_new, ey_up, ey_down, new_model_point
...
def data_augment(pts_aug_params, PC, gt_R, gt_t, gt_s, mean_shape, sym, aug_bb, aug_rt_t, aug_rt_r,
    ...
    def aug_3D_bc_with_flag(PC, gt_R, gt_t, gt_s, model_point, nocs_scale, mean_shape, flag):
        pc_new, s_new, ey_up, ey_down, model_point_new = defor_3D_bc_in_batch(PC, gt_R, gt_t, gt_s + mean_shape, model_point,
                                                                nocs_scale)
        pc_new = torch.where(flag.unsqueeze(-1), pc_new, PC)
        s_new = torch.where(flag, s_new - mean_shape, gt_s)
        return pc_new, s_new, ey_up, ey_down, model_point_new
    ...
    PC, gt_s, _, _, model_point = aug_3D_bc_with_flag(PC, gt_R, gt_t, gt_s, model_point, nocs_scale, mean_shape, flag)
    ...
    return PC, gt_R, gt_t, gt_s, model_point

def data_augment_contrast(pts_aug_params, PC, gt_R, gt_t, gt_s, mean_shape, sym, aug_bb, aug_rt_t, aug_rt_r,
                        model_point, nocs_scale, obj_ids, check_points=False):
    """
    PC torch.Size([32, 1028, 3])
    gt_R torch.Size([32, 3, 3])
    ...
    # prob_rt = torch.ones((bs, 1), device=PC.device)
    # flag = prob_rt < pts_aug_params['aug_rt_pro']
