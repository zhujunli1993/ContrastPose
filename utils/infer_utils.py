...
def compute_degree_cm_mAP(final_results, synset_names, log_dir, degree_thresholds=[360], shift_thresholds=[100], iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=False, eval_recon=False, plot_figure=True):
    ...
    bs = final_results['gt_RTs'].shape[0]
    ...
    gt_class_ids = final_results['gt_class_ids'].cpu().numpy().astype(np.int32)
    gt_RTs = np.array(final_results['gt_RTs'].cpu().numpy())
    # gt_scales = np.array(final_results['gt_scales'][i])
    gt_handle_visibility = final_results['gt_handle_visibility'].cpu().numpy()

    # pred_bboxes = np.array(final_results['pred_bboxes'][i])
    pred_class_ids = final_results['pred_class_ids'].cpu().numpy()
    # pred_scales = final_results['pred_scales'][i]
    # pred_scores = final_results['pred_scores'][i]
    pred_RTs = np.array(final_results['pred_RTs'].cpu().numpy())
    ...
    iou_aps = iou_3d_aps
    kind_result = {}
    kind_result["5 degree, 2 cm"] = "{:.1f}".format(pose_aps[-1, degree_thres_list.index(5), shift_thres_list.index(2)] * 100)
    kind_result["5 degree, 5 cm"] = "{:.1f}".format(pose_aps[-1, degree_thres_list.index(5), shift_thres_list.index(5)] * 100)
